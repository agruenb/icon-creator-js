...
export default class ReferenceImage extends Rect {
    ...
    constructor(x: number, y: number, width: number, height: number, url: string, aspectRatio: number = 1) {//aspectRatio is width/height
    ...
    markerEdited(marker: any, limit: any, xPrecise: number, yPrecise: number) {
        ...
        if (marker.memorize == "rotate") {
            ...
            let newWidth = this.pt(newHeight * parseFloat(this.ratio));
            changes = {
                width: newWidth,
                height: newHeight,
